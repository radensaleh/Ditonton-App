...
class _TVSeasonDetailPageState extends State<TVSeasonDetailPage> {
  ...
  void initState() {
    ...
    Future.microtask(
      () {
        context.read<SeasonDetailTvBloc>().add(FetchSeasonDetailTvShow(
            tvId: widget.tvId, seasonNumber: widget.seasonNumber));
        context.read<DetailTvBloc>().add(FetchDetailTvShow(id: widget.tvId));
        context
            .read<RecommendationTvBloc>()
            .add(FetchRecommendationTvShow(id: widget.tvId));
        context.read<WatchlistStatusTvCubit>().loadWatchlistStatus(widget.tvId);
      },
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: BlocBuilder<SeasonDetailTvBloc, SeasonDetailTvState>(
        builder: (context, state) {
          if (state is SeasonDetailTvLoadingState) {
            return const Center(child: CircularProgressIndicator());
          } else if (state is SeasonDetailTvHasDataState) {
            return SafeArea(
              child: DetailSeasonContent(
                season: state.seasonTv,
                tv: state.detailTv,
              ),
            );
          } else if (state is SeasonDetailTvErrorState) {
            return Center(child: Text(state.message));
          } else if (state is SeasonDetailTvInitialState) {
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            BlocBuilder<WatchlistStatusTvCubit,
                              ...
                              // if (state is WatchlistStatusTvInitial) {
